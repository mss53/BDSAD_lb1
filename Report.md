...
В отчёт рекомендуется включить скриншоты выполнения команд:

- **Рис. 1.** Вывод команды `jps` с запущенными компонентами NameNode, DataNode, ResourceManager, NodeManager;
- **Рис. 2.** Вывод команды `hdfs dfs -ls /user/hadoop/lab_01/input/` с загруженным CSV‑файлом.
...
Результат показывает отсутствие пропусков (`0` для всех колонок), что позволяет обойтись без дополнительной обработки `NULL`.
...
### 4.2. Интерпретация графика для бизнеса

С точки зрения бизнеса полученная визуализация позволяет:

- быстро сравнить отделы по средней производительности сотрудников;
- оценить, как уровень производительности соотносится со средним стажем (опытом) в каждом отделе;
- выявить отделы, где при сопоставимом стаже сотрудники демонстрируют более высокую эффективность (кандидаты на изучение лучших практик);
- обнаружить отделы, в которых относительно низкий стаж сочетается с высокой производительностью (возможный эффект интенсивного найма и адаптации).

В условиях реальных данных подобный анализ может быть использован для:

- корректировки HR‑политик (обучение, наставничество, удержание);
- оптимизации структуры подразделений;
- планирования численности и бюджета фонда оплаты труда.
